        ...
        for item in self.root.getchildren():
            for i in item.getchildren():
                if 'source' in i.keys() and 'target' in i.keys():
                    ...
                    edge_counter[(src_sw_dpid, int(i.attrib['source']))] = edge_counter[(
                        src_sw_dpid, int(i.attrib['source']))] + 1
        ...
        for item in self.root.getchildren():
            for i in item.getchildren():
                if 'source' in i.keys() and 'target' in i.keys():
                    ...
                    if (src_sw_dpid, dst_sw_dpid) not in links_dup_check and (
                            dst_sw_dpid, src_sw_dpid) not in links_dup_check:
                        final_topo[
                            ((src_sw_dpid, int(i.attrib['source'])), (dst_sw_dpid, int(i.attrib['target'])), 's')] = 1
            ...
            for (first, second, node_type) in sorted(self.topology_graph):
                if node_type == "h":
                    ...
                    if sys.version_info >= (3, 0):
                        hosts.append(net.addHost('h' + str(first[1] + 1), cls=Host, ip=str(
                            ipaddress.IPv4Address(int(ipaddress.IPv4Address(first_ip)) + first[1] + 1)),
                                                 defaultRoute=None))
                    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--availtopo', dest='avail_topo', help="prints list of all available topologies and exit.",
                        required=False, action="store_true")

    parser.add_argument('--toponame', dest='topo_name', help="Topology name e.g. Abilene", required=False, type=str)
    parser.add_argument('--cport', dest='controller_port', help="Controller port in mininet, default value is 6653.",
                        required=False, type=int, default=6653)
    parser.add_argument('--cip', dest='controller_ip', help="Controller ip in mininet, default value is 127.0.0.1.",
                        required=False, type=str, default="127.0.0.1")

    parser.add_argument('--controller', dest='controller_type',
                        help="Default controller is mininet controller, other options: remote,ovscontroller",
                        required=False, type=str, default="controller")
